...
class GbaPpu {
  ...
  renderMode0() {
    ...
    for (let y = 0; y < 160; y++) for (let x = 0; x < 240; x++) {
      ...
      const sprite = this.spritePixel(x, y, best); if (sprite) color = sprite;
  ...
  spritePixel(x, y, bgPriority) {
    ...
    for (let i = 127; i >= 0; i--) {
      ...
      let px = x - (sx >= 256 ? sx - 512 : sx); let py = y - (sy >= 160 ? sy - 256 : sy); if (px < 0 || py < 0 || px >= dim[0] || py >= dim[1]) continue;
