...
async function init() { const rom = await window.duopocket.getRom(); if (!rom) { document.querySelector('#status').textContent = 'ROM indisponível'; return; } emulator.value = new DuoGba(rom); document.querySelector('#status').textContent = 'ARM7TDMI · vídeo próprio'; loop(); }
