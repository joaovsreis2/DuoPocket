...
(() => {
  ...
  async function init() {
    ...
    worker.onmessage = (event) => {
      ...
      else if (message.type === "ready") document.querySelector("#status").textContent = "ARM7TDMI \xB7 59,7 FPS \xB7 Flash 1M";
      else if (message.type === "save") window.duopocket.saveRom(new Uint8Array(message.bytes));
    };
    worker.onerror = (error) => {
      console.error(error);
      document.querySelector("#status").textContent = "Erro no n\xFAcleo GBA";
    };
    ...
    setInterval(() => worker?.postMessage({ type: "save" }), 5e3);
  }
  init().catch((error) => {
    console.error(error);
